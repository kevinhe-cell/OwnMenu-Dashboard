...
export default function FacebookSchedulePostModal({
  show,
  onClose,
  onPosted,
}) {
  ...
  const handleFileUpload = async (e) => {
    ...
    for (const file of files) {
      ...
      if (fmt === "image" && isVideoFile(file)) {
        Swal.fire({
          icon: "info",
          title: t("marketing.instagram_post_modal.swal.format_error_title"),
          text: t("marketing.instagram_post_modal.swal.format_error_text_image"),
          timer: 2200,
      ...
      if (fmt === "video" && isImageFile(file)) {
        Swal.fire({
          icon: "info",
          title: t("marketing.instagram_post_modal.swal.format_error_title"),
          text: t("marketing.instagram_post_modal.swal.format_error_text_video"),
          timer: 2200,
          showConfirmButton: false,
        });
        continue;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/facebook-account/schedule", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      await Swal.fire({
        icon: "success",
        title: t("marketing.schedule_post.swal.success_title"),
        timer: 2000,
        showConfirmButton: false,
      });
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title className="fw-semibold fs-4">
          {t("marketing.schedule_post.title_fb")}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body className="bg-white px-4 pt-4 pb-3">
        <Form.Group className="mb-4">
          ...
          <div className="d-flex flex-wrap align-items-stretch gap-2 mb-2">
            ...
            >
              ...
              {t("marketing.instagram_post_modal.from_gallery")}
            </Button>
          </div>
          <Form.Text className="text-muted">
            {t("marketing.instagram_post_modal.upload_fb_text", { max: maxItems })}
          </Form.Text>
        </Form.Group>
        ...
        <Form.Group className="mt-4 mb-3">
          <Form.Label className="fw-semibold">
            {t("marketing.instagram_post_modal.caption_label")} <span className="text-muted small">{t("marketing.instagram_post_modal.caption_optional")}</span>
          </Form.Label>
