...
export default function InstagramPostModal({ show, onClose, onPosted }) {
  ...
  const postingSteps = t("marketing.instagram_post_modal.steps", { returnObjects: true });
  ...
  const handleFileUpload = async (e) => {
    ...
    for (const file of files) {
      if (!isImageFile(file) && !isVideoFile(file)) {
        Swal.fire({
          icon: "warning",
          title: t("marketing.instagram_post_modal.swal.unsupported_title"),
          text: t("marketing.instagram_post_modal.swal.unsupported_text", { name: file.name }),
        });
        continue;
      }
  ...
  const handleSubmit = async () => {
    if (mediaFiles.length === 0) {
      Swal.fire({
        icon: "warning",
        title: t("marketing.instagram_post_modal.swal.missing_content_title"),
        text: t("marketing.instagram_post_modal.swal.missing_content_text"),
      });
      return;
    }
    ...
    if (hasLargeFiles) {
      const result = await Swal.fire({
        icon: "info",
        title: t("marketing.instagram_post_modal.swal.large_file_title"),
        html: t("marketing.instagram_post_modal.swal.large_file_text"),
        showCancelButton: true,
        confirmButtonText: t("marketing.instagram_post_modal.swal.continue_btn"),
    ...
    try {
      ...
      const res = await fetch("/api/instagram-account/post", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      if (!res.ok) {
        ...
        return Swal.fire({
          icon: "error",
          title: t("marketing.instagram_post_modal.swal.upload_failed_title"),
          text: data.error || t("marketing.instagram_post_modal.swal.upload_failed_text"),
        });
      }
      ...
      if (hasLargeFiles) {
        await Swal.fire({
          icon: "success",
          title: t("marketing.instagram_post_modal.swal.success_large_title"),
          html: t("marketing.instagram_post_modal.swal.success_large_text"),
          timer: 5000,
        ...
        await Swal.fire({
          icon: "success",
          title: t("marketing.instagram_post_modal.swal.success_title"),
          timer: 2000,
  ...
  const regenerateCaption = async () => {
    ...
    if (isImageFile(firstFile) || isVideoFile(firstFile)) {
      ...
      Swal.fire({
        icon: "info",
        title: t("marketing.instagram_post_modal.regenerate_caption"),
    ...
    >
          ...
          <div className="px-4 py-3">
            ...
            <div className="text-center mt-2">
              <small className="text-muted">
                {t("marketing.instagram_post_modal.step_indicator", { current: currentStep + 1, total: postingSteps.length })}
              </small>
            </div>
          </div>
        </div>
      ...
      <Modal.Header closeButton>
        <Modal.Title className="fw-semibold fs-4">
          {t("marketing.instagram_post_modal.title_insta")}
        </Modal.Title>
      </Modal.Header>
        ...
        {hasLargeFiles && (
          <Alert variant="info" className="mb-3">
            <Alert.Heading className="h6 mb-1">
              <strong>{t("marketing.instagram_post_modal.large_file_alert")}</strong>
            </Alert.Heading>
            <p className="mb-0 small">
              {t("marketing.instagram_post_modal.large_file_desc")}
            </p>
          </Alert>
        )}
        ...
        <Form.Group className="mt-4 mb-4">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <Form.Label className="fw-semibold d-flex justify-content-between align-items-center">
              <span>{t("marketing.instagram_post_modal.caption_label")} <span className="text-muted small">{t("marketing.instagram_post_modal.caption_optional")}</span></span>
            </Form.Label>
